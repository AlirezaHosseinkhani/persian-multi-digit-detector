...
public abstract class BaseLoaderCallback implements LoaderCallbackInterface {
    ...
    public void onPackageInstall(final int operation, final InstallCallbackInterface callback)
    {
        switch (operation)
        {
            ...
            {
                final AlertDialog InstallMessage = new AlertDialog.Builder(mAppContext).create();
                InstallMessage.setTitle("پکیچ openCV یافت نشد");
                InstallMessage.setMessage(" لطفا برای استفاده از برنامه openCV Manager را نصب کنید");
                InstallMessage.setCancelable(false); // This blocks the 'BACK' button
                InstallMessage.setButton(AlertDialog.BUTTON_POSITIVE, "تایید", new OnClickListener()
                {
                    public void onClick(DialogInterface dialog, int which)
                    {
//                        callback.install();
